(function (window, document) {
    ...
    function FullPage(options) {
        ...
        var pageContain = document.getElementById(options.id),
            pageChildren = pageContain.children,
            pageChildrenLen = pageChildren.length,
            iPage = pageChildrenLen,
        ...
        if (!pageChildren || pageChildrenLen === 1) return;
        if (options.mode) {
            _isNav = options.mode.indexOf('nav:') !== -1;
            mode = options.mode.split(',');
            modeLen = mode.length;
        }
        for (_t = 0; _t < pageChildrenLen; _t++) {
            pageStyle.push(pageChildren[_t].style);
            stepArr.push(+pageChildren[_t].getAttribute('data-step') || 0);
            stepNow.push(0);
        }
            ...
            transform: function (o, from, to) {
                ...
                switch (o['translate']) {
                    case 'Y' :
                        ...
                        trans(pageChildren[to], 0, rangeNow, 0, o);
                        break;

                    case 'X' :
                        rangeNow = to > from ? pageRange.X : -pageRange.X;
                        trans(pageChildren[to], rangeNow, 0, 0, o);
                        break;
                    ...
                    case 'XY' :
                        ...
                        trans(pageChildren[to], rangeNow.X, rangeNow.Y, 0, o);
                        break;

                    default:
                        trans(pageChildren[to], 0, 0, 0, o);
                        break;
                }
                setTimeout(function () {
                    trans(pageChildren[to], 0, 0, slideTime, o);
                }, fix + 50);
            },
            opacity: function (o, from, to) {
                var s = pageChildren[to].style;
        ...
        if (options.continuous) {
            roundPage = function (pageChildren) {
                var i = pageChildrenLen;

                if (arguments[1]) {
                    if (pageChildren >= pageChildrenLen) {
                        while (i--) {
                            stepNow[i] = 0;
                        }
                    } else if (pageChildren < 0) {
                        while (i--) {
                            stepNow[i] = stepArr[i];
                        }
                    }
                ...
                return (pageChildrenLen + pageChildren % pageChildrenLen) % pageChildrenLen
            }
        }

        checkStep = function (direct) {
            var oldStep = stepNow[currentPage],
                newStep = oldStep + direct;

            if (newStep >= 0 && newStep <= stepArr[currentPage]) {
                if (newStep === 0) {
                    replaceClass(pageChildren[currentPage], 'step1', '');
                    stepNow[currentPage] = newStep;
                    return false;
                }
                if (newStep === 1 && oldStep === 0) {
                    pageChildren[currentPage].className += ' step1';
                    stepNow[currentPage] = newStep;
                    return false;
                }
                replaceClass(pageChildren[currentPage], 'step' + oldStep, 'step' + newStep);
                stepNow[currentPage] = newStep;
                return false;
            }
            return true;
        ...
        resetStep = function () {
            var stepTo = pageChildren[currentPage].getAttribute('data-step-restart');
            if (stepTo == null) return;

            stepTo = +stepTo > stepArr[currentPage] ? stepArr[currentPage] : +stepTo;
            if (stepTo === 0) {
                replaceClass(pageChildren[currentPage], 'step' + stepNow[currentPage], '');
            } else {
                replaceClass(pageChildren[currentPage], 'step' + stepNow[currentPage], 'step' + stepTo);
            }
            stepNow[currentPage] = stepTo;
        };
        ...
        goPage = function (to) {
            ...
            if (options.beforeChange) {
                if (options.beforeChange(currentPage, pageChildren[currentPage]) === 'stop') {
                    return;
                }
            }

            if (_isLocked                 			// make sure translate is already
                || to === currentPage) return;			// don't translate if thispage

            if (options.continuous) {
                to = roundPage(to, 1);
            } else {
                if (to >= pageChildrenLen
                    && stepNow[currentPage] >= stepArr[currentPage]
                    || to < 0
                    && stepNow[currentPage] === 0) return;
            }

            _isLocked = true;

            if (!arguments[1] && !checkStep(to - currentPage)) {
            ...
            for (_effectNow in effect) {
                resetAndRun[_effectNow](effect[_effectNow], currentPage, to);
            }

            fix += browser.cssCore === '' ? 20 : 0;
            indexOld = currentPage;
            currentPage = to;
            if (_isNav) navChange(indexOld, currentPage);
            setTimeout(function () {
                // fix for bug in ie6-9 about z-index
                pageChildren[to].className += ' slide';
            }, fix);

            setTimeout(function () {

                pageStyle[to][browser.cssCore + 'TransitionDuration'] = slideTime + 'ms';
            }, 20);

            setTimeout(function () {

                resetStep();
                replaceClass(pageChildren[indexOld], 'current', '');
                replaceClass(pageChildren[currentPage], 'slide', 'current');

                if (options.callback) {
                    options.callback(currentPage, pageChildren[currentPage]);
                }
                if (isMac) {
        ...
        init();
        // Tag first pageChildren
        _t = pageChildren[currentPage].className;
        pageChildren[currentPage].className = _t.indexOf('current') !== -1 ? _t : _t + ' current';
                ...
                switch (true) {
                    ...
                    case m === 'touch' :
                        ...
                        (function () {
                            ...
                            var pageIndexMax = pageChildrenLen - 1,
                            ...
                            if (effect.transform.translate === 'Y') {
                                setIndex = function () {
                                    ...
                                    if (prev) {
                                        ...
                                        pageChildren[pr].className += ' swipe';
                                    }
                                    if (next) {
                                        ...
                                        pageChildren[ne].className += ' swipe';
                                    }
                                };
                                move = function (o) {
                                ...
                                validReset = function (s, n) {
                                    ...
                                    if (options.continuous) {
                                        to = roundPage(to);
                                        _o = pageChildren[roundPage(currentPage - n)];
                                        _f = true;
                                    } else {
                                        _o = pageChildren[currentPage - n];
                                    }

                                    if (_o) {
                                        replaceClass(_o, 'swipe', '');
                                    }
                                    if (!_f && to < 0 || to > pageChildrenLen - 1) {
                                        setTimeout(function () {
                                            _isLocked = false;
                                        }, 50);
                                        return;
                                    ...
                                    if (_isNav) {
                                        navChange(currentPage, to);
                                    }

                                    s.opacity = 1;
                                    replaceClass(pageChildren[to], 'swipe', 'slide');
                                    s[browser.cssCore + 'TransitionDuration'] = _t + 'ms';
                                    s[browser.cssCore + 'Transform'] = 'translate(0,0) translateZ(0)';
                                    setTimeout(function () {
                                        resetStep();
                                        replaceClass(pageChildren[currentPage], 'current', '');
                                        replaceClass(pageChildren[to], 'slide', 'current');
                                        currentPage = to;
                                        if (options.callback) {
                                            options.callback(currentPage, pageChildren[currentPage]);
                                        }
                            ...
                            touchEvent = {
                                start: function (e) {
                                    ...
                                    if (options.onSwipeStart
                                        && options.onSwipeStart(currentPage, pageChildren[currentPage]) === 'stop') {
                                ...
                                move: function (e) {
                                    ...
                                    if (!isValidMove) {
                                        ...
                                        _t = _t === options.effect.transform['translate'];
                                        isValidMove = true;
                                    } else {
                                        ...
                                        if (stepNow[currentPage] + direct >= 0 && stepNow[currentPage] + direct <= stepArr[currentPage]) return;
                                        if (_t) move(delta);
                                    }
                                },
                                end: function (e) {
                                    ...
                                    if (!isValidSlide
                                        ...
                                        && options.beforeChange(currentPage, pageChildren[currentPage]) === 'stop') {
                                        if (prev) reset(prev, -1);
                                        if (next) reset(next, +1);
                                    } else if (nextDiff === -1) {
                                        validReset(prev, -1);
                    ...
                    case m.indexOf('nav:') !== -1 :
                        (function () {
                            ...
                            _t = navChildren[currentPage].className;

                            if (!navObj || !navChildren) return;

                            while (navLen--) {
                                // set attr for finding specific pageChildren
                                navChildren[navLen].setAttribute('data-pageChildren', navLen);
                            }
                            if (_t.indexOf('active') === -1) {
                                navChildren[currentPage].className = _t === '' ? 'active' : _t + ' active';
                            }
                            ...
                            gotoPage = function (e) {
                                ...
                                goPage(+e.getAttribute('data-pageChildren'), 1);
                            };
    ...
    if (typeof exports === 'object') module.exports = FullPage;
    else if (typeof define === 'function' && define.amd) define([], function () {
        return FullPage;
    });
    else window.FullPage = FullPage;
})(window, document);